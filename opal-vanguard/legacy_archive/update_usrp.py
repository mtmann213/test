...
mapper_old = """class FreqDiffMapper(gr.sync_block):
    ...
    def work(self, input_items, output_items):
        ...
        for i in range(len(in0)):
            # Reset phase reference at the start of every 48-carrier OFDM symbol
...
demapper_old = """class FreqDiffDemapper(gr.sync_block):
    def __init__(self):
    ...
    def work(self, input_items, output_items):
        in0, out = input_items[0], output_items[0]
        for i in range(len(in0)):
            # Reset reference carrier every 48 bits to match Mapper
            if self.count % 48 == 0: out[i] = 0; self.prev_sym = 1.0+0j
            else:
...
demapper_new = """class FreqDiffDemapper(gr.sync_block):
    def __init__(self):
        gr.sync_block.__init__(self, "FreqDiffDemapper", in_sig=[np.complex64], out_sig=[np.uint8])
