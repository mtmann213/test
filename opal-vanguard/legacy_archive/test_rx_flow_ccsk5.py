...
if __name__ == '__main__':
    ...
    delay = 0
    for i in range(200):
        if list(rx_data[i:i+64]) == tx_bits[:64] or list(rx_data[i:i+64]) == [1^x for x in tx_bits[:64]]:
            delay = i
            break
            
    print(f"Sync delay: {delay}")
    if delay > 0:
        match = sum(1 for a, b in zip(tx_bits, rx_data[delay:delay+len(tx_bits)]) if a == b or a == 1^b)
        print(f"Match rate (allowing inversion): {match/len(tx_bits)*100:.2f}%")
