...
p = MockPacketizer()
# Test with SYN packet from session_manager
import struct
syn_payload = struct.pack('>H', 0xACE).ljust(16, b'\x00')

# Need to set up correct meta for type 1 (SYN)
meta = pmt.make_dict()
meta = pmt.dict_add(meta, pmt.intern("type"), pmt.from_long(1))

msg_in = pmt.cons(meta, pmt.init_u8vector(len(syn_payload), list(syn_payload)))
...
class MockDepacketizer(depacketizer):
    def __init__(self):
        ...
        self.output_msg = None
        self.use_comsec = True
        self.comsec_key = bytes.fromhex(cfg['link_layer']['comsec_key'])
