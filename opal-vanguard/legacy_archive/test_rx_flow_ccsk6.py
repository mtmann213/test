...
class MockDepacketizer(depacketizer):
    ...
    def message_port_pub(self, port, msg):
        if pmt.symbol_to_string(port) == "out":
            self.output_msg = msg
            print("DEPACKETIZER SUCCESS:", pmt.u8vector_elements(pmt.cdr(msg)))
...
class test_tb(gr.top_block):
    def __init__(self):
        ...
        self.connect(self.chan, self.agc, self.bpsk_rx_filter, self.bpsk_sync, self.costas, self.demod_b, self.depkt)
